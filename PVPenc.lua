...
-- Função para ler o perfil com base no nome do personagem
scriptFuncs.readProfile = function(filePath, callback)
    if g_resources.fileExists(filePath) then
        ...
        -- Assegura que o 'trapspells' seja sempre uma tabela
        if type(result.trapspells) ~= 'table' then
            result.trapspells = {}  -- Inicializa como uma tabela vazia se não for
        end

        callback(result)  -- Passa o resultado para o callback
    else
        -- Se o arquivo não existir, chama o callback com uma tabela vazia
        callback({
            trapspells = {}  -- Inicializa 'trapspells' como uma tabela vazia
        })
    end
end


-- Função para salvar o perfil com base no nome do personagem
scriptFuncs.saveProfile = function(configFile, content)
    local status, result = pcall(function()
        return json.encode(content, 2)  -- Codifica o conteúdo em formato JSON com indentação
    end)
    ...
    -- Salva o arquivo no diretório específico do personagem
    g_resources.writeFileContents(configFile, result)
end

-- Inicializa a estrutura de dados para armazenar configurações por personagem
storageProfilesTraps[g_game.getCharacterName()] = {
    trapspells = {}  -- Armazenamento dos feitiços de armadilha para o personagem
}


-- Diretório base para o personagem
MAIN_DIRECTORY = "/bot/" .. modules.game_bot.contentsPanel.config:getCurrentOption().text .. "/storage/"

-- Usando o nome do personagem para o arquivo de configurações
...
-- Lê o perfil do personagem a partir do arquivo JSON
scriptFuncs.readProfile(STORAGE_DIRECTORY, function(result)
    -- Se o arquivo existe e os dados estão no formato correto, aloca os dados na estrutura
    storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()] = result or {
        trapspells = {}  -- Caso não exista, inicializa com valor padrão
    }
    resetCooldowns()  -- Reseta os cooldowns após carregar os dados
end)

-- Função para salvar os dados de configuração no arquivo
scriptFuncs.saveProfile(STORAGE_DIRECTORY, storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()])

-- Função para reindexar a tabela de feitiços
...
attachSpellWidgetCallbacks = function(widget, spellId, table)
  ...
  widget.onDragMove = function(self, mousePos, moved)
      ...
      if table[spellId] then
          ...
          scriptFuncs.saveProfile(STORAGE_DIRECTORY, storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()])
      end
      return true
  end
...
function isAnySelectedKeyPressed()
  for index, value in ipairs(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].keySpells) do
...
-- Inicializa a estrutura de dados para armazenar configurações por personagem
storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()] = {
  [g_game.getCharacterName()] = {  -- Chave baseada no nome do personagem
    trapspells = {}  -- Armazenamento dos feitiços de armadilha
  }
}
...
-- Função para resetar cooldowns
function resetCooldowns()
    if storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()] then
        if storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells then
            for _, spell in ipairs(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells) do
...
function canCasttrap()
    for key, value in ipairs(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells) do
...
function stopToCast()
    ...
    for index, value in ipairs(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells) do
...
function refreshtrapList(list, table)
    if table then
        ...
        for index, entry in ipairs(table) do
            ...
            attachSpellWidgetCallbacks(newWidget, entry.index, storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells)
            ...
            label.onDoubleClick = function(widget)
                ...
                for i, v in ipairs(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells) do
                    if v == entry then
                        removeTable(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells, i)
                    end
                end
...
trapInterface.moveUp.onClick = function()
  ...
  storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells[index].index = index - 1;
  storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells[index - 1].index = index;
  table.sort(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells, function(a,b) return a.index < b.index end)
  scriptFuncs.saveProfile(STORAGE_DIRECTORY, storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()]);
end
...
trapInterface.insertSpell.onClick = function(widget)
    ...
    local spellConfig = {
        index = #storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells + 1,
        spellCast = spellName,
    ...
    table.insert(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells, spellConfig)
    refreshtrapList(trapInterface, storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells)
    scriptFuncs.saveProfile(STORAGE_DIRECTORY, storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()])
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
macro(1, function()
    if not (trapspellsWidgets and storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells) then return; end
    
    for index, spellConfig in ipairs(storageProfilesTraps[g_game.getCharacterName()][g_game.getCharacterName()][g_game.getCharacterName()].trapspells) do
