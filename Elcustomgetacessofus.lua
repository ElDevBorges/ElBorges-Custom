...
local urls = {
    "http://38.46.142.218:8080/Elcustom/1_FiltroBattleenc.lua",
...
local maxRetries = 3  -- Número máximo de tentativas por script
local retryCount = 0
...
local function loadNextScript()
    ...
    print("Carregando script [" .. currentIndex .. "/" .. #urls .. "]: " .. url)

    -- Faz a requisição HTTP para buscar o script
    modules.corelib.HTTP.get(url, function(script)
        if not script or script == "" then
            ...
            if retryCount < maxRetries then
                ...
                print("Tentando novamente (" .. retryCount .. "/" .. maxRetries .. ")...")
                loadNextScript()  -- Tenta carregar novamente
            else
                print("Falha ao carregar o script " .. currentIndex .. " após " .. maxRetries .. " tentativas.")
                retryCount = 0  -- Reseta para o próximo script
        ...
        -- Tenta carregar e executar o script
